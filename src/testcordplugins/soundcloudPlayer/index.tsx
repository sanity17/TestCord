...
function parseTracks(data: any): ScTrack[] {
    ...
    for (const item of data.collection) {
        ...
        // Priority 2: hls (m3u8 - better supported by modern browsers)
        if (!streamUrl) {
            for (const tc of transcodings) {
...
async function getStreamUrl(streamUrl: string, clientId: string): Promise<string> {
    ...
    // If it's already a final stream URL or direct HLS
...
async function refreshTrackData(track: ScTrack, clientId: string): Promise<ScTrack> {
    try {
        ...
        // Priority 1: progressive (direct MP3) - le plus stable
        for (const tc of transcodings) {
            if (tc.format?.protocol === "progressive" && tc.url) {
                streamUrl = tc.url;
                break;
...
// ─── Player singleton (persists after modal close) ─────────────────
...
function SoundCloudModal({ onClose }: { onClose: () => void; }) {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                // FIX: MediaEngineStore.getOutputDevices() returns Discord's internal IDs,
                // NOT the real WebAudio deviceIds required by setSinkId().
                // We use navigator.mediaDevices.enumerateDevices() to get the real deviceIds.
                ...
                if (outputs.some(d => !d.label)) {
                    try {
                        // Requesting microphone access triggers permission to list outputs too
                        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
                        stream.getTracks().forEach(t => t.stop());
